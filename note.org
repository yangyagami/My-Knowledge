...
#+begin_src bash
  ...
  DESKTOP_NAME=$(basename "$DESKTOP_DIR")
#+end_src
...
* 内网端口访问问题
127.0.0.1 只能本机器访问，想要其他机器访问需要绑定 0.0.0.0 。


* 使用 patchelf 事后修改 libA.so 的 RUNPATH
如果你没有 libA.so 的源码或不想重新编译，可以用 patchelf 工具添加：
#+begin_src bash
  patchelf --set-rpath '$ORIGIN' libA.so
#+end_src
...
* 什么是 init ramfs
init ramfs 是 initial ram file system（初始内存文件系统），它是一个简单的文件系统，存储在内存中。内核用它来加载初始进程。这是现代 Linux 发行版加载第一个进程的方式。
